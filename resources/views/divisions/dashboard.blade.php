            ...
            <div class="card-header">
                <ul class="nav nav-tabs card-header-tabs" id="divisionTabs" role="tablist">
                    <li class="nav-item" role="presentation">
                        <button class="nav-link active" id="quality-tab" data-bs-toggle="tab" data-bs-target="#quality" type="button" role="tab">
                                ...
                                <div class="col-md-6 col-lg-4 mb-3">
                                    <div class="card wip-card quality_check h-100">
                                        <div class="card-body">
                                            ...
                                            <div class="mb-3">
                                                <span class="badge bg-secondary">Route #{{ $wip->partOperation->route_order }}</span>
                                                <span class="badge bg-info">{{ number_format($wip->wip_qty) }} pcs</span>
                                            </div>

                                            @if($wip->started_at)
                                                <small class="text-muted d-block mb-2">
                                                    <i class="bi bi-clock"></i> Started: {{ $wip->started_at->format('d M Y, H:i') }}
                                                </small>
                                            @endif
                                            ...
                                            <div class="d-grid gap-2">
                                                @if($wip->status == 'waiting')
                                                    <button class="btn btn-primary btn-sm" onclick="startWip({{ $wip->id }}, 'quality')">
                                                ...
                                                @endif
                                                <a class="btn btn-outline-secondary btn-sm" href="{{ route('wip-trackings.show', $wip->id) }}">
                                ...
                                <div class="col-md-6 col-lg-4 mb-3">
                                    <div class="card wip-card process h-100">
                                        <div class="card-body">
                                            ...
                                            <div class="d-grid gap-2">
                                                @if($wip->status == 'waiting')
                                                    <button class="btn btn-primary btn-sm" onclick="startWip({{ $wip->id }}, 'process')">
                                                        <i class="bi bi-play"></i> Start Process
                                                    </button>
                                                @elseif($wip->status == 'in_progress')
                                                    <button class="btn btn-success btn-sm" onclick="completeWip({{ $wip->id }})">
                                                        <i class="bi bi-check2"></i> Complete
                                                    </button>
                                                @endif
                                                <a href="{{ route('wip-trackings.show',$wip->id) }}" class="btn btn-outline-secondary btn-sm">
                                                    <i class="bi bi-eye"></i> View Details
                                                </a>
                                            </div>
                                        </div>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <small class="text-muted">{{ Str::limit($wip->partInternal->part_name, 30) }}</small>
                                        </td>
                                        <td>
                                            <span class="badge bg-secondary">#{{ $wip->partOperation->route_order }}</span>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <a href="{{ route('wip-trackings.show', $wip->id) }}" class="btn btn-sm btn-outline-secondary">
                                                <i class="bi bi-eye"></i>
                                            </a>
                                        </td>
                                    </tr>
