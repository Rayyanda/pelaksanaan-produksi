                                    ...
                                    <div class="info-value">
                                        @if ($poProduction->due_date)
                                            ...
                                            <div class="mt-2">
                                                ...
                                                @elseif($daysLeft <= 7)
                                                    <span class="badge bg-warning">
                                                        <i class="bi bi-clock"></i> {{ round($daysLeft) }} days remaining
                                                    </span>
                                                @else
                                                    <span class="badge bg-success">
                                                        <i class="bi bi-check-circle"></i> {{ round($daysLeft) }} days
                                                        remaining
                                                    </span>
                                                @endif
                                            </div>
                                ...
                                @if ($fieldCount > 0)
                                    <div class="alert alert-light mb-3">
                                        <div class="d-flex justify-content-between">
                                            <span><i class="bi bi-info-circle"></i> Total Fields:
                                                <strong>{{ $fieldCount }}</strong></span>
                                            <span>Last Updated:
                                                <strong>{{ $poProduction->updated_at->diffForHumans() }}</strong></span>
                                        </div>
                                    </div>
                                @endif
                    ...
                    <div class="card">
                        <div class="card-body">
                            <div class="d-flex justify-content-between">
                                ...
                                <div>
                                    <a href="{{ route('po-productions.edit', $poProduction->id) }}"
                                        class="btn btn-warning">
                                        <i class="bi bi-pencil"></i> Edit
                                    </a>
                                    <form action="{{ route('po-productions.destroy', $poProduction->id) }}" method="POST"
                                        class="d-inline"
    ...
    <script>
            ...
            fetch('{{ route('po-productions.refresh-snapshot', $poProduction->id) }}', {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        document.getElementById('snapshotData').innerHTML = '<pre class="mb-0">' + JSON.stringify(data
                            .data, null, 4) + '</pre>';
                        ...
                        statusDiv.innerHTML =
                            '<div class="alert alert-danger alert-dismissible fade show"><i class="bi bi-x-circle"></i> ' +
                            data.message +
                            '<button type="button" class="btn-close" data-bs-dismiss="alert"></button></div>';
                    }
                })
                .catch(error => {
                    statusDiv.innerHTML =
                        '<div class="alert alert-danger alert-dismissible fade show"><i class="bi bi-x-circle"></i> Failed to refresh: ' +
                        error.message +
                        '<button type="button" class="btn-close" data-bs-dismiss="alert"></button></div>';
                });
        }
        ...
        function copySnapshot() {
            ...
            navigator.clipboard.writeText(snapshotText).then(() => {
                ...
                const statusDiv = document.getElementById('refreshStatus');
                statusDiv.innerHTML =
                    '<div class="alert alert-success alert-dismissible fade show"><i class="bi bi-check-circle"></i> Snapshot copied to clipboard!<button type="button" class="btn-close" data-bs-dismiss="alert"></button></div>';
