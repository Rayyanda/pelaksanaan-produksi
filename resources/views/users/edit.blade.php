                            ...
                            <form action="{{ route('users.update', $user->id) }}" method="POST">
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group mb-4">
                                            ...
                                            <select class="form-select choices @error('role') is-invalid @enderror"
                                                ...
                                                <option value="ppc"
                                                    {{ old('role', $user->role) == 'ppc' ? 'selected' : '' }}>PPC</option>
                                                <option value="foreman"
                                                    {{ old('role', $user->role) == 'foreman' ? 'selected' : '' }}>Foreman
                                                </option>
                                                <option value="supervisor produksi"
                                                    {{ old('role', $user->role) == 'supervisor produksi' ? 'selected' : '' }}>
                                                    Supervisor Produksi</option>
                                ...
                                @if ($user->id == auth()->id())
                                    <div class="alert alert-warning">
                                        ...
                                        <strong>Warning:</strong> You are editing your own account. Be careful not to lock
                                        yourself out!
                                    </div>
                                @endif
...
@push('scripts')
    ...
    <script>
        ...
        function trackChanges() {
            ...
            if (changes.length > 0) {
                tracker.innerHTML = '<ul class="list-unstyled mb-0">' + changes.map(c => '<li class="mb-1">' + c + '</li>')
                    .join('') + '</ul>';
            } else {
